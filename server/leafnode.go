...
func establishHTTPProxyTunnel(proxyURL, targetHost string, timeout time.Duration, username, password string) (net.Conn, error) {
	...
	if username != "" && password != "" {
		...
		authBuilder.WriteString(password)
		auth := base64.StdEncoding.EncodeToString([]byte(authBuilder.String()))

		connectReq.WriteString("Proxy-Authorization: Basic ")
		connectReq.WriteString(auth)
		connectReq.WriteString("\r\n")
	}
	...
	// Set deadline for reading the proxy response
	if err := conn.SetReadDeadline(time.Now().Add(timeout)); err != nil {
		conn.Close()
		return nil, fmt.Errorf("failed to set read deadline: %v", err)
	}

	// Use bufio.Reader to carefully read only the HTTP response headers
	// This is crucial to avoid consuming data intended for the remote server
	...
	if err != nil {
		conn.Close()
		return nil, fmt.Errorf("failed to read proxy status line: %v", err)
	}

	statusStr := strings.TrimSpace(statusLine)
	if !strings.HasPrefix(statusStr, "HTTP/1.1 200") && !strings.HasPrefix(statusStr, "HTTP/1.0 200") {
		conn.Close()
		return nil, fmt.Errorf("proxy CONNECT failed: %s", statusStr)
	}

	// Read headers until we find the empty line that ends the HTTP response
	for {
	...
	// Clear the read deadline now that we've finished reading the proxy response
	if err := conn.SetReadDeadline(time.Time{}); err != nil {
		conn.Close()
		return nil, fmt.Errorf("failed to clear read deadline: %v", err)
	}

	// Important: Now we need to return a connection that doesn't have the buffered reader
	// attached to it, because the buffered reader may have consumed extra data beyond
	// the HTTP headers. We need to create a connection wrapper that handles this.

	// Check if the reader has buffered any extra data beyond the HTTP headers
	if reader.Buffered() > 0 {
		// If there's buffered data, we need to create a connection wrapper that
		// will first serve the buffered data, then the underlying connection
	...
	// If no extra data was buffered, we can return the connection directly
	return conn, nil
}

// proxyTunnelConn wraps a connection to handle any data that was buffered
// during HTTP proxy response parsing but belongs to the tunneled connection
...
func (ptc *proxyTunnelConn) Read(b []byte) (n int, err error) {
	// First, serve any data that was buffered during proxy response parsing
	if !ptc.bufferConsumed && ptc.bufferedReader.Buffered() > 0 {
		n, err = ptc.bufferedReader.Read(b)
		if ptc.bufferedReader.Buffered() == 0 {
			ptc.bufferConsumed = true
		}
		return n, err
	}

	// Once buffered data is consumed, read directly from the underlying connection
	return ptc.Conn.Read(b)
}
